...
def generate_per_file(model, settings, singleFile, smoothVelocities, genre):
	...
	# Setup I/O
	output_file = 'generations/' + settings.filename + "/" + settings.filename + "_" + str(settings.generate_length) + ".mid"
	count = 0
	...
	for t, tick in enumerate(generated_encoding):
		for p, val in enumerate(tick):
			...
			elif (t != 0) and (abs(generated_encoding[t][p] - generated_encoding[t-1][p]) < 10) and smoothVelocities:
